...
class QELModel:
    ...
    def calculate_observable_expectations(self, input_vec: NDArray[np.double]) -> NDArray[np.double]:
        """Calculates the list of observable values after dynamics given an array_like input of size (n_features) and initial state"""
    ...
    def batch_calculate_observable_expectations(self, input_vecs: NDArray[np.double]) -> NDArray[np.double]:
        """Calculates observable values for an array_like batch of input vectors of size (n_samples, n_features)."""
        outputs = []
    ...
    def fit(self, X: NDArray[np.double], y: NDArray[np.double]) -> None:
        """X is an array_like of (n_samples, n_features), fit trains a model to predict a new output"""
        prepared_data = self.batch_calculate_next_observable_list(X)
        check_X_y(prepared_data, y, multi_output=True)
        self.subestimator.fit(prepared_data, y)
        self.is_fitted_ = True

    def predict(
        self, X: NDArray[np.double]) -> NDArray[np.double]:
        """X is an array_like of (n_samples, n_features), returns the predicted output shape (n_samples,)"""
        check_array(X)
        check_is_fitted(self, "is_fitted_")
        expections = self.calculate_observable_expectations(X)
