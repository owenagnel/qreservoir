...
from qreservoir.reservoirs.IsingMagTraverseReservoir import IsingMagTraverseReservoir
from qreservoir.encoders.HEEncoder import HEEncoder


def test_no_encoder_raises_error() -> None:
    with pytest.raises(ValueError):
        reservoir = IsingMagTraverseReservoir(None, 3, 2)
        reservoir.get_reservoir_state(np.zeros(3), None)


def test_circuit_dimensions() -> None:
    reservoir = IsingMagTraverseReservoir(None, 3, enc_qubit_num=2)
...
def test_get_reservoir_state_with_encoder() -> None:
    ...
    reservoir = IsingMagTraverseReservoir(encoder, 3)
...
def test_connecting_incorrectly_sized_encoder_raises_error() -> None:
    encoder = HEEncoder(2, 1)
    reservoir = IsingMagTraverseReservoir(None, 3, 3)
    with pytest.raises(ValueError):
